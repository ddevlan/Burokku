package co.burokku.books.command;

import co.burokku.books.util.ClassUtils;
import com.google.common.base.Preconditions;
import com.google.common.collect.ImmutableSet;
import lombok.Getter;
import co.burokku.books.BooksPlugin;
import co.burokku.books.command.param.Parameter;
import co.burokku.books.command.param.ParameterData;
import co.burokku.books.command.param.ParameterType;
import co.burokku.books.command.param.defaults.*;
...
public final class CommandHandler implements Listener {
    ...
    public static void init() {
        BooksPlugin.getInstance().getLogger().info("Initializing command handler...");
        // Only allow the CoreCommandHandler to be initiated once.
        // Note the '!' in the .checkState call.
        Preconditions.checkState(!initiated);
        initiated = true;

        BooksPlugin.getInstance().getServer().getPluginManager().registerEvents(new CommandHandler(), BooksPlugin.getInstance());
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                try {
                    ...
                    Field commandMapField = BooksPlugin.getInstance().getServer().getClass().getDeclaredField("commandMap");
                    commandMapField.setAccessible(true);

                    Object oldCommandMap = commandMapField.get(BooksPlugin.getInstance().getServer());
                    CommandMap newCommandMap = new CommandMap(BooksPlugin.getInstance().getServer());
                    ...
                    commandMapField.set(BooksPlugin.getInstance().getServer(), newCommandMap);
                } catch (Exception e) {
        ...
        CommandHandler.loadCommandsFromPackage(BooksPlugin.getInstance(), "co.burokku.books.command.commands");
        BooksPlugin.getInstance().getLogger().info("Command handler initialized");
    }
    ...
    public static void loadCommandsFromPackage(Plugin plugin, String packageName) {
        ...
        int i = 0;
        BooksPlugin.getInstance().getLogger().info("Loading commands from '" + plugin.getName() + "'...");
        for (Class<?> clazz : ClassUtils.getClassesInPackage(plugin, packageName)) {
            registerClass(clazz);
            i++;
        }
        BooksPlugin.getInstance().getLogger().info("Loaded " + i + " commands from '" + plugin.getName() + "' in " + (System.currentTimeMillis() - start) + "ms.");
    }
     ...
     */
    public static void registerParameter(Class<?> transforms, ParameterType parameterType) {
    ...
    protected static void registerMethod(Method method) {
        ...
        if (!Modifier.isStatic(method.getModifiers())) {
            BooksPlugin.getInstance().getLogger().log(Level.SEVERE, "Method '" + method.getName() + "' cannot be registered as a command because it is not static.");
            return;
        }
        ...
        for (int parameterIndex = 1; parameterIndex < method.getParameterTypes().length; parameterIndex++) {
            ...
            if (parameterAnnotation != null) {
                ...
                BooksPlugin.getInstance().getLogger().warning("Method '" + method.getName() + "' has a parameter without a @Parameter annotation.");
                return;
            }
        }
    ...
    public static CommandData evalCommand(final CommandSender sender, String command) {
        ...
        if (found.isAsync()) {
            ...
            }.runTaskAsynchronously(BooksPlugin.getInstance());
        } else {
